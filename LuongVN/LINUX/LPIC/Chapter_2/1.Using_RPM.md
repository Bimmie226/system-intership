...
![alt text](../images/2_4_21.png)

- Ví dụ về nội dung của file `rocky.repo`

![alt text](../images/2_4_22.png)

- Cấu trúc của tệp `.repo`

| **Khóa (Key)**        | **Mô tả**                                              | **Ví dụ (giống định dạng trong ảnh)**                                                            |
| --------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------ |
| `[repo_id]`           | Chuỗi định danh duy nhất cho kho lưu trữ (repository). | `[appstream]``                                                               |
| `name`                | Tên mô tả dễ hiểu của kho.                             | `name=Rocky Linux $releasever - AppStream`                                                       |
| `baseurl`             | Đường dẫn trực tiếp đến kho lưu trữ.                   | `#baseurl=http://dl.rockylinux.org/$contentdir/$releasever/AppStream/$basearch/os/`              |
| `mirrorlist`          | URL trả về danh sách mirror của repo.                  | `mirrorlist=https://mirrors.rockylinux.org/mirrorlist?arch=$basearch&repo=AppStream-$releasever` |
| `enabled`             | Xác định có bật repo này hay không.                    | `enabled=1`                                                                                      |
| `countme`             | Gửi tín hiệu thống kê số hệ thống dùng repo.           | `countme=1`                                                                                      |
| `metadata_expire`     | Thời gian hết hạn metadata của repo.                   | `metadata_expire=6h`                                                                             ||
| `gpgcheck`            | Kiểm tra chữ ký GPG của các gói.                       | `gpgcheck=1`                                                                                     |
| `gpgkey`              | Đường dẫn đến khóa GPG dùng để xác thực gói.           | `gpgkey=file:///etc/pki/rpm-gpg/RPM-GPG-KEY-Rocky-9`                                             |
