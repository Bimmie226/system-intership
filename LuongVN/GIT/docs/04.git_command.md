...
### 7.1 `git checkout -B <branch>`

```bash
git checkout -B develop
```

- Nếu branch develop chưa được tạo ra thì lúc này nó sẽ tạo mới develop. Còn nếu develop đã được tạo ra rồi thì branch develop sẽ được đưa lại về HEAD

**Ví dụ:**

- Đây là các commit hiện tại của nhánh master:

    ![alt text](../images/checkout_01.png)

- Tôi sử dụng lệnh sau để đưa về commit "upload file 2 nhanh master cua dev2"

    ![alt text](../images/checkout_02.png)

    ```bash
    git checkout bfd0156652c71e987c0158a8bfe9a6e7f53ca181
    ```

    ![alt text](../images/checkout_03.png)

- Tôi đang ở detached HEAD, tôi muốn nhánh master được kéo về HEAD này sử dụng lệnh sau:

    ```bash
    git checkout -B master 
    ```

    ![alt text](../images/checkout_04.png)
    
    Ta thấy, lúc này nhánh master đã được đưa về trạng thái lúc "upload file 2 nhanh master cua dev2" 

### 7.2 `git checkout <branch> -- <file>`

Lấy file từ branch khác về 

**Ví dụ:**

- Đây là các file trên nhánh develop:

    ![alt text](../images/checkout_05.png)

- Đây là các file hiện tại của master:

    ![alt text](../images/checkout_06.png)

- Ta muốn lấy file `file_1_develop.txt` từ develop về master ta sẽ sử dụng câu lệnh sau:

    ![alt text](../images/checkout_07.png)
...
- Sửa 1 file trên nhánh master:

    ![alt text](../images/checkout_08.png)

- Muốn quay lại trạng thái commit mới nhất ta sử dụng câu lệnh sau:

    ```bash
    git checkout -- file_1_master.txt
    ```

    ![alt text](../images/checkout_09.png)

### 7.4 `git checkout <commit> -- <file>`

Lấy file từ commit cũ (rollback) mà không ảnh hưởng đến file khác 

```bash
git checkout 21e527f -- main.cpp
```

### 7.5 `git checkout -- .`

Reset toàn bộ file chưa commit về trạng thái commit gần nhất

**NOTE:** Nguy hiểm vì bạn có thể mất code khi chưa commit 

### 7.6 `git checout --orphan <new_branch>`

Tạo branch không liên kết với bất kì branch nào trước đó

![alt text](../images/checkout_10.png)

## 8. git fetch

`git fetch` dùng để lấy (download) dữ liệu mới từ remote repository về local, nhưng **KHÔNG** tự động merge vào branch hiện tại

**Hiểu đơn giản:**

- Nó cập nhật thông tin mới nhất từ remote 
- Nhưng không làm thay đổi code bạn đang làm

**Một số mục đích chính khi sử dụng git fetch:**

- `Xem trước thay đổi từ remote`: Bạn có thể xem người khác đã push gì mà không ảnh hưởng code của mình
- `Tránh xung đột (conflict)`: So với git pull (fetch + merge), thì fetch giúp bạn kiểm tra trước, merge thủ công => an toàn hơn
- `Cập nhật remote-tracking branch`: Ví dụ bạn có: origin/main, origin/dev thì sau khi fetch các branch này sẽ được cập nhật
...
- Git sẽ:
    1. Kết nối tới remote (`origin`)
    2. So sánh commit local với remote
    3. Tải về commit mới
    4. Cập nhật các branch dạng:
    ...
    - **KHÔNG** merge vào `master` local
    - **KHÔNG** thay đổi working directory

**Phân biệt branch và origin/branch:**

- `branch`: 
  - Là branch local - tức là branch bạn đang làm việc trên máy tính
  - Bạn có thể commit, merge, rebase trực tiếp nào nó
  - Đại diện cho code bạn đang phát triển
- `Origin/branch`:
  - Là remote-tracking branch - tức là bản sao của branch trên remote (origin) mà local biết được
  - Bạn không thể commit trực tiếp vào `origin/master`
  - Nó chỉ được cập nhật khi bạn chạy `git fetch` hoặc `git pull`.
  - Giúp bạn biết remote master hiện tại có những commit gì mà branch local chưa có.
...
- Ta kiểm tra log trên nhánh origin/master:

    ![alt text](../images/fetch_01.png)

- Ta kiểm tra log trên nhánh master:

    ![alt text](../images/fetch_02.png)

- Ta fetch thử nhánh `origin/master`:

    ![alt text](../images/fetch_03.png)

- Ta kiểm tra lại log trên nhánh origin/master:

    ![alt text](../images/fetch_04.png)

    => Ta thấy lúc này trên origin/master đã có thểm 1 commit mới 

- Ta kiểm tra log trên nhánh master:

    ![alt text](../images/fetch_05.png)

    => Ta thấy trên nhánh master vẫn chỉ có 3 commit thôi

    ![alt text](../images/fetch_06.png)

### 8.0 `--all`
Fetch tất cả các remote 

```bash
git fetch --all
```

Dùng khi bạn có nhiều remote: `origin`, `upstream`, ...

### 8.1 `--prune`

Xóa các branch remote đã bị xóa trên server

```bash
git fetch --prune
```

- Remote đã xóa branch `feature-x`
- Local vẫn còn `origin/feature-x`
...
![alt text](../images/fetch_07.png)

### 8.3 Fetch và map vào branch cụ thể

```bash
git fetch origin master:my_master
```

- Lấy `origin/master`
- Gán vào branch `my_master`
...
Tìm hiểu về cơ chế và các loại git merge [tại đây](https://github.com/Bimmie226/system-intership/blob/main/LuongVN/GIT/docs/05.git_merge.md)

### 9.0 Kiểm soát merge strategy

| Option      | Ý nghĩa                                                                |
| ----------- | ---------------------------------------------------------------------- |
| `--no-ff`   | Luôn tạo merge commit, dù có thể fast-forward. Giữ lại lịch sử rõ ràng |
| `--ff-only` | Chỉ merge nếu có thể là fast-forward, không tạo merge commit           |
| `--squash`  | Gộp tất cả commit của branch vào thành 1 commit duy nhất               |


### 9.1 Kiểm soát commit

| Option         | Ý nghĩa                                               |
| -------------- | ----------------------------------------------------- |
| `--no-commit`  | Merge nhưng không tự động tạo commit, để review trước |
| `-m "message"` | Đặt message tùy chỉnh cho merge commit                |

### 9.2 Strategy options(`-s` vs `-X`)

```bash
git merge -s ours feature-branch        # Giữ nguyên code hiện tại, bỏ code của branch merge
git merge -X ours feature-branch        # Khi conflict, ưu tiên code của branch hiện tại 
git merge -X theirs feature-branch      # Khi conflict, ưu tiên code của branch được merge vào
```
...
| Option       | Ý nghĩa                                                                                                                               |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| `--abort`    | Hủy merge đang dở, quay về trạng thái trước                                                                                           |
| `--continue` | Tiếp tục merge sau khi đã giải quyết conflict (Chỉ dùng khi Git có thông báo `You have not concluded your merge (MERGE_HEAD exists)`) |
