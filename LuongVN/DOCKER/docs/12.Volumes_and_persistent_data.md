...
- `docker volume create`: tạo volume mới. Mặc định, volume được tạo với driver `local`, option `-d` để chỉ định driver khác 
- `docker volume ls`: liệt kê các volume trên Docker host
- `docker volume inspect`: hiển thị thông tin chi tiết về volume
- `docker volume prune`: xóa tất cả các volume không được sử dụng bởi bất kỳ container hoặc service replica nào
- `docker volume rm`: xóa các volume cụ thể không còn được sử dụng 
- `docker plugin install`: dùng để cài đặt các volume plugin mới từ Docker Hub.
- `docker plugin ls`: liệt kê tất cả các plugin đã được cài đặt trên Docker Host 
