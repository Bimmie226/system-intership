...
2 máy VM ở khác dải mạng

### 2.0 Mô hình

![alt text](../images/lab_11.png)

### 2.1 Mục tiêu

Kiểm chứng xem `VM01: 192.168.123.200` có ping được tới `VM02: 192.168.122.136` hay không?

### 2.2 Thực hiện

- Thông tin của máy host `KVM01`:

    ![alt text](../images/lab_12.png)

- Thông tin của máy `VM01` trên host `KVM01`:

    ![alt text](../images/lab_13.png)
...
- Từ máy `VM01: 192.168.123.200` ta sẽ thực hiện ping sang `VM02: 192.168.122.136`:

    ![alt text](../images/lab_14.png)

    -> Không thể ping từ VM01 sang VM02

### 2.3 Suy luận lý thuyết

Khi `VM01` ở dải `192.168.123.0/24 ` ping đến `VM02` ở dải `192.168.122.0/24` nó thấy chúng khác dải nhau vì thế VM01 sẽ gửi gói tin đến gateway và thực hiện NAT sang IP của host KVM tức là `192.168.174.35` tuy nhiên lúc này vì không có routing từ host KVM01 đến VM02 dó đó gói tin chết ngay tại đây

### 2.4 Kiểm chứng

Ta sẽ bắt gói tin ở host KVM01 để kiểm chứng:

```bash
tcpdump -i any -n -w kvm01.pcap
```

Ta sẽ tập trung vào 3 gói tin sau:

![alt text](../images/lab_15.png)

Lý do có 2 gói tin giống nhau `15528` và `15529` là vì tôi bắt gói tin này trên tất cả interface của host KVM01 do đó gói tin ở `vnet0` và `virbr1` đều bị bắt 
...
- (1):
    ...
    ```bash
    192.168.123.200 → 192.168.122.136
    ```

    - Gói gốc từ VM xuất hiện ở `vnet1`, `virbr1`
- (2):

    ```bash
    192.168.174.35 → 192.168.122.136
    ```

    - Sau khi SNAT: source IP bị đổi thành `192.168.174.35` đúng như lập luận của ta ở bên trên 
