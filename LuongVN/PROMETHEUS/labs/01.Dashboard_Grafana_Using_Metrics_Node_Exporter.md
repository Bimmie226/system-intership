...
- [Mục lục](#mục-lục)
- [Dashboard cho giám sát trên Grafana sử dụng metric Node Exporter](#dashboard-cho-giám-sát-trên-grafana-sử-dụng-metric-node-exporter)
  - [I. Sử dụng Dashboard có sẵn cho Node Exporter](#i-sử-dụng-dashboard-có-sẵn-cho-node-exporter)
    - [1.0 Tổng quan về Dashboard Node Exporter Full (ID: 1860)](#10-tổng-quan-về-dashboard-node-exporter-full-id-1860)
    - [1.1 Thực hành import Dashboard 1860](#11-thực-hành-import-dashboard-1860)
  - [II. Vẽ Dashboard cho metric Node Exporter](#ii-vẽ-dashboard-cho-metric-node-exporter)
    - [2.0 Mục tiêu](#20-mục-tiêu)
    - [2.1 Thực hiện](#21-thực-hiện)
      - [Bước 1: Tạo Dashboard](#bước-1-tạo-dashboard)
      - [Bước 2: Thêm Variable](#bước-2-thêm-variable)
      - [Bước 3: Query cho Panel 1 - CPU Usage](#bước-3-query-cho-panel-1---cpu-usage)
      - [Bước 4: Query cho Panel 2 - Load Average](#bước-4-query-cho-panel-2---load-average)
      - [Bước 5: Query cho Panel 3 - CPU Core Count](#bước-5-query-cho-panel-3---cpu-core-count)
      - [Bước 6: Query cho Panel 4 - Memory Usage](#bước-6-query-cho-panel-4---memory-usage)
      - [Bước 7: Query cho Panel 5 - Filesystem Space Available](#bước-7-query-cho-panel-5---filesystem-space-available)
      - [Bước 8: Query cho Panel 6 - Filesystem Used](#bước-8-query-cho-panel-6---filesystem-used)
      - [Bước 9: Query cho Panel 7 - Network Traffic](#bước-9-query-cho-panel-7---network-traffic)
      - [Bước 10: Query cho Panel 8 - Network Saturation](#bước-10-query-cho-panel-8---network-saturation)
    - [2.2 Tổng quan về Dashboard](#22-tổng-quan-về-dashboard)
...
- Ta sẽ sử dụng 2 metric là: `node_network_receive_bytes_total` và `node_network_transmit_bytes_total`
  ...
  ```promql
  rate(node_network_receive_bytes_total{instance="$instance"}[5m]) * 8
  ```

  ```promql
  rate(node_network_transmit_bytes_total{instance="$instance"}[5m]) * 8
  ```
...
#### Bước 10: Query cho Panel 8 - Network Saturation 

- Tôi muốn xem phần trăm về lượng băng thông đang sử dụng của từng interface so với lượng băng thông tối đa
- Ta sử dụng tương tự như Panel 7 tuy nhiên ta sẽ dùng thêm metric là `node_network_speed_bytes`: tốc độ tối đa của một interface mạng

  ```promql
  (node_network_speed_bytes{instance="$instance"} > bool 0) * (rate(node_network_receive_bytes_total{instance="$instance"}[$__rate_interval]) / node_network_speed_bytes{instance="$instance"}) 
  ```

  ```promql
  (node_network_speed_bytes{instance="$instance"} > bool 0) * (rate(node_network_transmit_bytes_total{instance="$instance"}[$__rate_interval]) / node_network_speed_bytes{instance="$instance"}) 
  ```

![alt text](../images/dashboard_25.png)


### 2.2 Tổng quan về Dashboard

![alt text](../images/dashboard_26.png)

![alt text](../images/dashboard_27.png)